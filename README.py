...
if menu == '🎤 MV yêu thích':
    ...
    tabA,tabB,tabC,tabD,tabE,tabF,tabG,tabH,tabI = st.tabs(["Game tung xúc sắc", "Game đoán số", "Kéo - Búa - Bao", "Game tính toán nhanh", "Game đuổi hình bắt chữ", "Multiple choice quiz", "Lucky number gamble", "Prizes gamble game", "10 question quiz"])
    with st.sidebar:
    ...
    with tabE:
        ...
        st.write(f"Điểm: {st.session_state.score1}")
        puzzles = [
            {
        ...
        if st.button("🔄 Vòng mới"):
            ...
            st.rerun()
    with tabF:
        st.header("Multiple choice game")
        questions = [
            {
                "question": "Which of the below is correctly spelled?",
                "option": ["aluminiom", "budderfly", "territory", "fosphorus"],
                "answer": "territory"
            },
            {
                "question": "Which of the below is correctly spelled?",
                "option": ["vulcano", "respiration", "plazma", "hecker"],
                "answer": "respiration"
            },
            {
                "question": "Which of the below is correctly spelled?",
                "option": ["command", "anvel", "pienapple", "creatiev"],
                "answer": "command"
            },
            {
                "question": "Which of the following people is not a Greek god?",
                "option": ["Perseus", "Hermes", "Thalia", "Medusa"],
                "answer": "Hermes"
            },
            {
                "question": "How many countries are there in Southeast Asia?",
                "option": ["11", "9", "18", "16"],
                "answer": "11"
            },
            {
                "question": "What does ⛏️ stand for?",
                "option": ["shovel", "anvil", "hook", "pickaxe"],
                "answer": "pickaxe"
            }
        ]
        ...
        st.write(f"Remaining time: {remaining_time} seconds")
        st.progress(st.session_state.quiz_index/len(questions))
        if st.session_state.quiz_done:
            st.success(f"All done. Your score: {st.session_state.quiz_score}/{len(questions)}")
            if st.button("Replay"):
                st.session_state.quiz_index = 0
                st.session_state.quiz_score = 0
                st.session_state.quiz_done = False
            ...
            st.header(f"Question {st.session_state.quiz_index+1}: {q['question']}")
            choice = st.radio(
                "Choose the answer",
                q['option'],
            ...
            if st.session_state.quiz_feedback != "":
                if st.button("Next question"):
                    ...
                    if st.session_state.quiz_index >= len(questions):
                        st.session_state.quiz_index = True
                    st.rerun()
    with tabG:
        st.title("Lucky number gamble")
        if "prize" not in st.session_state:
            st.session_state.prize = []
        new_prizes = st.text_input("Enter the prizes")
        if st.button("➕ Add prize"):
            if new_prizes:
                st.session_state.prizes.append(new_prizes)
        st.write("Prizes list: ", st.session_state.prize)
        if st.button("Roll"):
            if st.session_state.prizes:
                result = random.choice(st.session_state.prize)
                st.success(f"Congratulation! You rolled out {result}")
                st.balloons()
            else:
                st.warning("You haven't add any prizes")
        if st.button("Reset"):
            st.session_state.prize = []
    with tabH:
        st.title("Lucky number rolling game")
        if "newprizes" not in st.session_state:
            st.session_state.newprizes = []
        if "chance" not in st.session_state:
            st.session_state.chance = []
        st.subheader("Add prizes")
        col1, col2 = st.columns(2)
        with col1:
            newprizes = st.text_input("Prize name")
        with col2:
            chance = st.number_input("Chance of rolling out the prize (%)", 1, 100, 1)
        if st.button("Add"):
            if newprizes:
                st.session_state.newprizes.append(newprizes)
                st.session_state.chance.append(chance)
                st.success(f"Added {newprizes}")
        st.subheader("Prizes list")
        if st.session_state.newprizes:
            for i, prize in enumerate(st.session_state.newprizes):
                st.write(
                    f"{i + 1}. {prize} | Chance: {st.session_state.chance[i]}%"
                )
        else:
            st.info("No prizes yet")
        st.subheader("Roll number")
        if st.button("Roll now"):
            if st.session_state.newprizes:
                spin_placeholder = st.empty()
                for i in range(15):
                    spin_placeholder.markdown(
                        f'Rolling... {random.choice(st.session_state.newprizes)}'
                    )
                    time.sleep(0.1)
                result = random.choices(
                    st.session_state.newprizes,
                    weights = st.session_state.chance,
                    k = 1
                )[0]
                spin_placeholder.empty()
                st.balloons()
                st.success(f"Congratulation, you rolled {result}")
            else:
                st.warning("No prizes have been added")
